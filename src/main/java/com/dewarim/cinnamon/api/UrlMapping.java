...
public enum UrlMapping {
    ...
    FOLDER__GET_FOLDER_BY_PATH("folder", "getFolderByPath", "/api", "Fetch a folder specified by its complete path. The path parameter must not end with a forward slash '/'.", FolderPathRequest.class, FolderWrapper.class),
    FOLDER__GET_FOLDER_BY_RELATIVE_PATH("folder", "getFolderByRelativePath", "/api", "Fetch a folder specified by its relative path. The relative path must not start or end with a forward slash: '/'.", FolderByRelativePathRequest.class, FolderWrapper.class),
