...
<indexItem>
  ...
  <searchString>/objectSystemData/aclId</searchString>
...
+
In this example, we select the ACL id via `/objectSystemData/aclId` (for folders, the root element is `folder`). If you want to index the title element of an XML document, you could use something like `/objectSystemData/content//title` to select it - see the "OSD XML" section below for the structure of the document that XPath expressions are evaluated against.
+
